...
async function jupiterOrder(params: Record<string, string>) {
  ...
  const res = await fetch(`${baseUrl}/swap/v2/order?${query}`, { headers: { 'x-api-key': apiKey } });
...
async function jupiterExecute(signedTransaction: string, requestId: string) {
  const { apiKey, baseUrl } = config();
  const res = await fetch(`${baseUrl}/swap/v2/execute`, {
    method: 'POST',
...
async function sellPositionToUsdc(position: TradingPositionRow): Promise<{ exitPrice: number; pnlUsdc: number; signature: string }> {
  const cfg = config();
  const connection = new Connection(cfg.rpcUrl, 'confirmed');
  ...
  return { exitPrice, pnlUsdc, signature: result.signature };
}
...
function recordClose(position: TradingPositionRow, status: 'closed_manual' | 'closed_stop', close: { exitPrice: number; pnlUsdc: number; signature: string }) {
  ...
  logTransaction({ type: close.pnlUsdc >= 0 ? 'earning' : 'expense', amount: Math.abs(close.pnlUsdc), description: `Closed Solana position ${position.symbol || position.output_mint} (${status}): ${close.pnlUsdc >= 0 ? '+' : ''}$${close.pnlUsdc.toFixed(2)}`, module: 'solana-trading' });
...
export async function closePosition(positionId: string): Promise<string> {
  ...
  if (!position) return `No position found with id ${positionId}.`;
  if (position.status !== 'open') return `Position ${positionId} is already ${position.status}.`;
  const close = await sellPositionToUsdc(position);
  recordClose(position, 'closed_manual', close);
  return `Closed ${position.symbol || position.output_mint}: ${close.pnlUsdc >= 0 ? '+' : ''}$${close.pnlUsdc.toFixed(2)} P&L. Signature: ${close.signature}`;
}
    ...
    for (const position of open) {
      try {
        ...
        if (currentPrice <= position.stop_loss_price) {
          const close = await sellPositionToUsdc(position);
          recordClose(position, 'closed_stop', close);
          closed++;
        }
      } catch (error: any) {
