...
export function keypairFromSecret(raw: string, envVarName: string): Keypair {
  const trimmed = raw.trim();
  if (trimmed.startsWith('[')) {
    const bytes = JSON.parse(trimmed);
    if (!Array.isArray(bytes)) throw new Error(`${envVarName} must be a JSON byte array, a base58 string, or a seed phrase`);
    return Keypair.fromSecretKey(Uint8Array.from(bytes));
  }
  if (trimmed.includes(' ')) {
    if (!bip39.validateMnemonic(trimmed)) throw new Error(`${envVarName} looks like a seed phrase but is not a valid BIP39 mnemonic — check for typos or extra spaces.`);
...
export function signerFromEnv(): Keypair {
  const raw = process.env.SOLANA_OPERATIONAL_SECRET_KEY;
  if (!raw) throw new Error('SOLANA_OPERATIONAL_SECRET_KEY is required for live transfers');
  return keypairFromSecret(raw, 'SOLANA_OPERATIONAL_SECRET_KEY');
}

/** Reconciles a USDC operational wallet. Live mode moves only the excess above the fixed cap.
 *  Runs inside withWalletLock so it can never race a trading buy/sell on the same wallet. */
export async function reconcileSolanaSurvival() {
  ...
  reconciling = true;
  try {
    return await withWalletLock(async () => {
      ...
      const feePayer = process.env.SOLANA_FEE_PAYER_SECRET_KEY
        ? keypairFromSecret(process.env.SOLANA_FEE_PAYER_SECRET_KEY, 'SOLANA_FEE_PAYER_SECRET_KEY')
        : signer;
